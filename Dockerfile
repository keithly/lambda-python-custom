...
RUN dnf upgrade -y && dnf install -y wget gnupg2 dirmngr dnf-plugins-core --allowerasing
RUN dnf builddep -y python3
WORKDIR /opt

RUN wget https://www.python.org/ftp/python/${PYTHON_VERSION}/Python-${PYTHON_VERSION}.tgz.asc
RUN wget https://www.python.org/ftp/python/${PYTHON_VERSION}/Python-${PYTHON_VERSION}.tgz
RUN gpg --keyserver hkps://keys.openpgp.org --recv-keys 64E628F8D684696D
RUN gpg --verify Python-${PYTHON_VERSION}.tgz.asc Python-${PYTHON_VERSION}.tgz
